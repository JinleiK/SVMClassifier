...
public class IncreamentalTrain {private double	delta_b=0;	//阈值增量
private double eps=0.001;   //Lagrange乘子的允许的误差
private int RAND_MAX=300;
private double alph[]=new double[svset.svAndEpage.size()];//Lagrange乘子
private double eError[]=new double[svset.svAndEpage.size()];//误差
private int target[]=new int[svset.svAndEpage.size()];//记录网页的类别，它只能为1和-1，通过SetSort函数来实现
 ...
 * 构造函数，初始化相关的参数
 */
public IncreamentalTrain()
{
	for(int i=0;i<svset.svAndEpage.size();i++)
	{
...
public void learning(int sort)
{
	...
	double tempLagrange[]=new double[svset.svAndEpage.size()];
	...
	while(( numChanged > 0 || examineAll ) && numTT < 150 )// 
	{
		...
		{
			for( int t = 0; t <svset.svAndEpage.size() ; t++ )
		...
		//progressBar.setValue(numTT + sort * 150);
...
public int exameTrainingSet(int i1)
{
	...
	if(( r1 < - GlobalVariable.tolerance && alph1 < GlobalVariable.tolerance ) || ( r1 > GlobalVariable.tolerance && alph1 > 0 ))//不满足KKT条件
	{
		...
		//寻找|E1-E2|最大的，
		{
			...
			for( k = 0; k <svset.svAndEpage.size() ; k++ )
		...
		//其中,RAND_MAX = 0x7fff
		{
			...
			//产生k0
			k0 = (int)(( Math.random() / RAND_MAX ) * svset.svAndEpage.size()  );

			//循环查找i2
			for( k = k0; k < svset.svAndEpage.size() + k0; k++ )
			{
				i2 = k % svset.svAndEpage.size();
